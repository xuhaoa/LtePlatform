...
(function(global) {
    ...
    var sourceFromStacktrace = function(offset) {
        var error = new Error();
    ...
    */
    ...
    var extend = function(a, b, undefOnly) {
        for (var prop in b) {
    ...
    var registerLoggingCallbacks = function(obj) {
        var i,
            l,
            key,
            callbackNames = [
    ...
    var runLoggingCallbacks = function(key, args) {
        var i, l;
    ...
    var verifyLoggingCallbacks = function() {
        var loggingCallback, userCallback;

        for (loggingCallback in loggingCallbacks) {
            if (loggingCallbacks.hasOwnProperty(loggingCallback)) {
    ...
    var setHook = function(module, hookName) {
    ...
    var asyncTest = function(testName, expected, callback) {
    ...
    var generateHash = function(module, testName) {
        var hex,
            i = 0,
            hash = 0,
    ...
    var focused;
    var test = function(testName, expected, callback, async) {
        if (focused) {
            return;
        }
        if (arguments.length === 2) {
    ...
    var only = function(testName, expected, callback, async) {
        if (focused) {
            return;
        }
    ...
    extend(QUnit, {
        ...
        module: function(name, testEnvironment, executeNow) {
            var createModule = function() {
                ...
                var moduleName = parentModule !== null && parentModule !== undefined ? [parentModule.name, name].join(" > ") : name;
                var module = {
                    name: moduleName,
            ...
            var setCurrentModule = function(module) {
    ...
    var priorityFill = function(callback) {
        var queue = config.queue.slice(priorityFill.pos);
        var prioritizedQueue = config.queue.slice(0, -config.queue.length + priorityFill.pos);
    ...
    var synchronize = function(callback, priority) {
        var last = !priority;
    ...
    Test.prototype = {
        ...
        queue: function() {
            ...
            var priority = QUnit.config.reorder && defined.sessionStorage &&
    ...
    */
    ...
    priorityFill.pos = 0; // Will be exposed as QUnit.asyncTest
 ...
 */
	 ...
	 */
        DiffMatchPatch.prototype.DiffMain = function(text1, text2, optChecklines) {
            ...
            var diffs = this.diffCompute(text1, text2, checklines, deadline);
